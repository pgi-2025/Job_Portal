...
def company_test_stats():
    """Dashboard stat: total students, and how many have actually logged in
    at least once (profiles.last_login set). Label on the frontend still
    reads "Students Attended Test" per request, but the number now reflects
    real logins, not test submissions."""
    if _require_company() is None:
        return error("Company profile not found.", 403)
    ...
    attended_test = logged_in.count if logged_in.count is not None else len(logged_in.data or [])

    return jsonify({"total_users": total_users, "attended_test": attended_test})
...
def company_applications():
    ...
    students = (
        supabase_admin.table("profiles")
        .select("id, full_name, college, degree, age, resume_url, photo_url")
        .in_("id", student_ids)
        .execute()
    ).data or []
